...
import React from "react";import{useRef,useEffect,useState,useCallback}from"react";const useIsStaticRenderer = () => false; const addPropertyControls = () => {}; const ControlType = {};const defaultImage={src:"https://framerusercontent.com/images/GfGkADagM4KEibNcIiRUWlfrR0.jpg",alt:"Gradient 1 - Blue"};/**
 * @framerSupportedLayoutWidth any-prefer-fixed
 * @framerSupportedLayoutHeight any-prefer-fixed
 *
 * @framerDisableUnlink
 */export default function LiquidImage(props){const{sourceType="image",image=defaultImage,video="https://framerusercontent.com/assets/MLWPbW1dUQawJLhhun3dBwpgJak.mp4",strength=.15,speed=.18,fit="cover",style,hotspots=[],borderRadius=8}=props;const isVideo=sourceType==="video"&&!!video;const wrapperRef=useRef(null);const canvasRef=useRef(null);const videoElRef=useRef(null);const offCanvasRef=useRef(null)// reused offscreen canvas (perf)
...
const fs=`
        ...
        `;function createShader(type,src){let s=gl.createShader(type);gl.shaderSource(s,src);gl.compileShader(s);return s;}function createProgram(vs,fs){let p=gl.createProgram();gl.attachShader(p,vs);gl.attachShader(p,fs);gl.linkProgram(p);return p;}function setup(){if(!gl)return;// Create shaders
